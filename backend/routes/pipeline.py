...
async def api_start_pipeline(project_id: str):
...
async def api_run_stage(project_id: str, stage_num: int):
    ...
    return {"ok": True, "stage": stage_num}


@router.get("/projects/{project_id}/stages")
def api_get_stages(project_id: str):
    project = get_project(project_id)
    if not project:
        raise HTTPException(404, "Project not found")
...
def api_get_stage_assets(project_id: str, stage_num: int):
